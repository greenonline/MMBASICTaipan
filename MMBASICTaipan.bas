...
820 GOSUB 180: GOSUB 860: GOSUB 190:GOSUB 1340:DN = INT ((C / 2) * RND (1)):IF RND (1) > .8 AND TR = 0 AND L <> 0 THEN VTAB=12:PRINT @(0*FONTX,12*FONTY) "A MESSENGER FROM ";LY$;" ASKS": PRINT "THAT YOU RETURN TO HONG KONG": PRINT "WITHOUT DELAY, TAIPAN." :GOSUB 780

821 REM IF RND (0) > .8 AND TR = 0 AND L <> 0 THEN PRINT "WITHOUT DELAY, TAIPAN." :GOSUB 780
