...
2 OPTION ESCAPE: CLRSCRN$="\027[2J":FONTX = MM.INFO(FONTWIDTH) : FONTY = MM.INFO(FONTHEIGHT):REM MMBASIC SPECIFICS
...
51 DIM INTEGER LO(9) = (21,14,7,0,35,49,56,42,84,200)
52 D = 1000: V=1860: GT = 1: C=400:MW = 50:SH = MW:SR = 1: G=1:VISITS(0) = 1: GOSUB 5000: KEY$ = "":GOSUB 590: HOME=0:PRINT CLRSCRN$:GOTO 120
...
130 VTAB=1: HTAB=1:PRINT @(1*FONTX,1*FONTY) "PORT ";LOC$(L);: HTAB=28:PRINT TAB(28) MON$(M);". ";DA+1;",";Y


141 VTAB=2: INVERSE=0:PRINT @(0*FONTX,2*FONTY) "CASH ";: Q = C:GOSUB 1330
...
310 PRINT CHR$ (8);: INVERSE=0:PRINT " ";:NORMAL=0:GOSUB 60:IF LEN (NUMBER$) > 0 AND ASC (KEY$) = 8 THEN PRINT KEY$;: PRINT" "; :PRINT KEY$;KEY$;:PRINT" ";:FG=1:IF LEN (NUMBER$) = 1 THEN NUMBER$ = "": FG = 0:GOTO 310

311 IF FG = 1 THEN NUMBER$ = LEFT$ (NUMBER$, LEN (NUMBER$) - 1):FG = 0: GOTO 310
312 IF ASC (KEY$) = 65 OR ASC (KEY$) = 13 THEN RETURN
313 IF ASC (KEY$) < 48 OR ASC (KEY$) > 57 THEN GOTO 310
314 NUMBER$ = NUMBER$ + KEY$: PRINT CHR$ (8);KEY$; : INVERSE=0:PRINT " ";:NORMAL=0: GOTO 310
320 IF X= 1 THEN IF KEY$ = "A" OR KEY$="a" THEN PRINT CHR$ (8);: INVERSE=0:PRINT "ALL"; :NUM = INT (C / GP(X1))
321 IF X = 1 AND KEY$ <> "A" AND KEY$ <> "a" THEN NUM = VAL (NUMBER$)
330 IF X = 2 THEN IF KEY$ = "A" OR KEY$="a" THEN PRINT CHR$ (8) ; : INVERSE=0:PRINT "ALL";:NUM = SG(X1)
...
360 GOSUB 130: GOSUB 1340:VTAB=11:INVERSE=0:PRINT @(0*FONTX,11*FONTY) ABLANK$:NORMAL=0:PRINT " T)RADE, R)ECORDS" ; :IF L = 0 THEN PRINT ", L)ENDER, G)ODOWN OR E)MBARK?":PRINT: PRINT ABLANK$
...
375 IF((KEY$ <> "G" AND KEY$ <> "g") OR L<>0) AND KEY$ <> "T" AND KEY$<> "R" AND KEY$ <> "L" AND KEY$ <> "E" AND KEY$ <> "t" AND KEY$<> "r" AND KEY$ <> "l" AND KEY$ <> "e" THEN GOSUB 770: GOTO 370
...
420 NUMBER$ = "":VTAB=13:PRINT @(0*FONTX,13*FONTY) ABLANK$:VTAB=13:PRINT @(0*FONTX,13*FONTY) " MOVE HOW MUCH ";GOODS$(X1);"? ";: GOSUB 310:IF KEY$ = "A" OR KEY$ = "a" THEN INVERSE=0:PRINT CHR$ (8) ;"ALL"; :NUM = SG(X1) :NORMAL=0
...
450 NUMBER$ = "": VTAB=13:PRINT @(0*FONTX,13*FONTY) BLANK$;:TAB 1:PRINT " MOVE HOW MUCH " ;GOODS$(X1);" TO SHIP? ";: GOSUB 310:IF KEY$ = "A" OR KEY$ = "a" THEN PRINT CHR$ (8);:INVERSE=0:PRINT "ALL" ;:NORMAL=0:NUM = GG(X1)
...
470 GOSUB 130: GOSUB 1340:VTAB=12:PRINT @(0*FONTX,12*FONTY) "[WANCHAI DISTRICT OF HONGKONG: HOME OF "; W$;"]":PRINT W$;" GREETS YOU, TAIPAN, AND WISHES YOU WELL.":GOSUB 780:VTAB=14:PRINT @(0*FONTX,14*FONTY) ABLANK$:PRINT ABLANK$
...
500 GOSUB 60:IF KEY$ = "B" OR KEY$ = "b" THEN LD$ = "BORROW": LD = 1
501 IF KEY$ = "P" OR KEY$ = "p" THEN LD$ = "PAY": LD = 2
...
520 GOSUB 1340: VTAB=15:PRINT @(0*FONTX,15*FONTY) "HOW MUCH DO YOU WISH TO ";LD$;",":PRINT "TAIPAN? ";:GOSUB 310:NUM = VAL (NUMBER$)


530 IF (KEY$ = "A" OR KEY$ = "a") AND LD = 1 THEN PRINT CHR$ (8);: INVERSE=0:PRINT "ALL"; : NORMAL=0:NUM = C * 2
531 IF (KEY$ = "A" OR KEY$ = "a") AND LD = 2 THEN PRINT CHR$ (8);: INVERSE=0:PRINT "ALL";: NORMAL=0:NUM = D
540 IF LD = 2 THEN GOTO 560
...
580 C = C - NUM: D = D - NUM:GOSUB 130:VTAB=15:PRINT @(0*FONTX,15*FONTY) W$;" ACCEPTS YOUR PAYMENT WITH GRATITUDE, TAIPAN";:GOSUB 780: GOTO 490
...
600 HOME=0:PRINT CLRSCRN$: INVERSE=0:PRINT ABLANK$;:NORMAL=0:PRINT "    RECORDS OF HIGH & LOW PRICES":INVERSE=0:PRINT ABLANK$;:NORMAL=0:PRINT:PRINT "    RECORDS BY P)ORT OR I)TEM? ": PRINT"    __________________________"


601 VTAB=10: PRINT @(0*FONTX,10*FONTY) "    PRESS <SPACEBAR> WHEN FINISHED":PRINT: INVERSE=0: PRINT ABLANK$: NORMAL=0
610 GOSUB 60:IF KEY$ = "P" OR KEY$ = "p" THEN GOSUB 720: GOTO 620
611 IF KEY$ = "I" OR KEY$ = "i" THEN PRINT: VTAB=5:PRINT @(0*FONTX,5*FONTY) ABLANK$;: PRINT ABLANK$: GOSUB 660: GOTO 670
...
630 HOME=0:PRINT CLRSCRN$:PRINT " ";LOC$(X);" PRICES (VISITS:";VISITS(X);")": INVERSE=0:PRINT " ITEM              HIGH         LOW     ";:NORMAL=0:FOR I = 0 TO 5:PRINT " ";GOODS$(I);:HTAB=21: PRINT TAB(21);
...
660 VTAB=5: PRINT @(0*FONTX,5*FONTY);:FOR I = 0 TO 5:PRINT " ";GOODS$(I),:NEXT I:PRINT:PRINT " WHAT ITEM (";TC$; "  ?": PRINT:PRINT ABLANK$: INVERSE=0: PRINT ABLANK$: NORMAL=0: GOSUB 260: RETURN
670 HOME=0:PRINT CLRSCRN$:PRINT " ";GOODS$(X1);"  PRICES": INVERSE=0:PRINT " PORT      VISITS     HIGH    LOW       ";:NORMAL=0:FOR I = 0 TO 9:PRINT LOC$(I);

680 VTAB=I+3: HTAB=12:PRINT @(12*FONTX,(I+3)*FONTY) VISITS(I);:IF LOW(I,X1) = 0 THEN HTAB=21: INVERSE=0: PRINT TAB(21) " ";: NORMAL=0:PRINT "  ?";: HTAB=32:PRINT TAB(32) "?":NEXT I:GOTO 690
681 IF LOW(I,X1)<>0 THEN HTAB=21: INVERSE=0:PRINT TAB(21) " ";:NORMAL=0:PRINT "  ";HIGH(I,X1) ;: HTAB=31:PRINT TAB(31) " ";LOW(I,X1): NEXT I
...
731 IF SH >= 0 THEN HOME=0:PRINT CLRSCRN$: PRINT TAB( 11);:INVERSE=0:PRINT "EMBARKING":NORMAL=0:PRINT TAB( 9);"FROM " ;LOC$ (L ) : INVERSE=0:PRINT ABLANK$:NORMAL=0:FOR I = 0 TO 9:IF L = I THEN NEXT I:GOTO 740
...
820 GOSUB 180: GOSUB 860: GOSUB 190:GOSUB 1340:DN = INT ((C / 2) * RND (1)):IF RND (1) > .8 AND TR = 0 AND L <> 0 THEN VTAB=12:PRINT @(0*FONTX,12*FONTY) "A MESSENGER FROM ";LY$;" ASKS": PRINT "THAT YOU RETURN TO HONG KONG"

821 IF RND (0) > .8 AND TR = 0 AND L <> 0 THEN PRINT "WITHOUT DELAY, TAIPAN." :GOSUB 780

830 IF C > 100 AND TR = 0AND L = 0 THEN GOSUB 1340:VTAB=12:PRINT @(0*FONTX,12*FONTY) YS$;", LIEUTENANTS":PRINT "OF THE MARINER, ";LY$;", ASK IF": PRINT "YOU WILL DONATE ";:Q = DN:GOSUB 1330:PRINT "TO THE TEMPLE OF TIN HAU, THE"
...
841 IF C > 0 AND TR = 0 AND L = 0THEN IF KEY$ <> "Y" AND KEY$ <> "y" THEN GOSUB 1340: VTAB=12:PRINT @(0*FONTX,12*FONTY) " ":PRINT " ";YS$;" DEPART":PRINT "ABRUPTLY IN A CHILLY SILENCE.": PRINT ABLANK$;
...
980 HOME=0:PRINT CLRSCRN$: PRINT:INVERSE=0:PRINT ABLANK$;:NORMAL=0:PRINT " SEA VOYAGE FROM ";LOC$(L) : PRINT " TO ";LOC$(PO) :INVERSE=0: PRINT ABLANK$: NORMAL=0: GOSUB 780: HOME=0:PRINT CLRSCRN$:ET = ABS (LO(L) - LO(PO))
981 IF (INT ( RND (1) * 50) + 1) > (INT ( RND (1) * ET) + 1) THEN GOTO 1290

985 REM STORM (990-1022)
990 IF RND (1) > .5 THEN GOTO 1030
991 VTAB=13: PRINT @(0*FONTX,13*FONTY) BLANK$;:HTAB=1:PRINT TAB(1) " STORM, TAIPAN!" : GOSUB 760
1000 IF RND (1) > .2 THEN GOTO 1020
1001 VTAB=14:PRINT @(0*FONTX,14*FONTY) " ANY PORT IN A STORM,                 ": GOSUB 760:PO = INT ( RND (1) * 10) :IF PO = L THEN VTAB=14: PRINT @(0*FONTX,14*FONTY) " WE CAN'T MAKE IT,                      TAIPAN,            ": SR = 0:GOTO 1090
1010 VTAB=14:PRINT @(0*FONTX,14*FONTY) " WE'RE HEADED                           FOR SHELTER,          ": GOSUB 780
1020 VTAB=14:PRINT @(0*FONTX,14*FONTY) " WE RODE OUT THE                        STORM, TAIPAN!       ": GOSUB 780
...
1290 CR = 0: SR = 1: L=PO : VISITS(L) = VISITS(L) + 1 : B=0: K=0 : GOSUB 160:HOME=0:PRINT CLRSCRN$:PRINT:INVERSE=0:PRINT @(0*FONTX,0*FONTY) ABLANK$;: NORMAL=0
...
1300 HOME=0:PRINT CLRSCRN$:NW = C - D:Q = NW / GT: VTAB=4: INVERSE=0: PRINT @(0*FONTX,4*FONTY) ABLANK$;: NORMAL=0: PRINT:PRINT "YOUR SCORE, BASED UPON TIME AND YOUR": PRINT "NET WORTH (EXCLUDING STOCK) IS ";: GOSUB 1330:INVERSE=0: PRINT ABLANK$: NORMAL=0
1310 IF KEY$ = "R" OR KEY$ = "r" THEN PRINT "WOULD YOU LIKE TO PICK UP THIS":PRINT"GAME WHERE YOU LEFT OFF (Y/N)?": GOSUB 60:IF KEY$ = "Y" THEN HOME=0:PRINT CLRSCRN$:GOTO 120
...
1335 PRINT INT (Q);Q$;: NORMAL=0:PRINT "      "
1337 RETURN
...
5890 NEXT A1
5910 NEXT A
5930 VTAB=14: PRINT @(0*FONTX,14*FONTY) ABLANK$;
5930 GOSUB 5950
5940 RETURN
...
6250 FOR B = 0 TO 13
6260 HTAB=40 - A: VTAB=B+1:PRINT @((40-A)*FONTX,(B+1)*FONTY) " "; LEFT$ (CH$(B),A); CHR$ (A);" ";
6270 NEXT B: NEXT A
6260 RETURN
